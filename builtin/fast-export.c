...
static char *reencode_message(const char *in_msg,
			      size_t in_encoding_len,
			      const char *in_encoding)
{
	static struct strbuf in_encoding_buf = STRBUF_INIT;

	strbuf_reset(&in_encoding_buf);
	strbuf_addf(&in_encoding_buf, "%.*s", (int)in_encoding_len, in_encoding);
			  ...
			  struct string_list *paths_of_changed_objects)
{
	...
	if (anonymize) {
		...
		case REENCODE_YES:
			reencoded = reencode_message(message, encoding_len, encoding);
			break;
		case REENCODE_NO:
			break;
		case REENCODE_ABORT:
