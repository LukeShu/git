...
static enum sign_mode signed_commit_mode = SIGN_WARN_STRIP;
...
static const char *find_commit_signature(const char *begin, const char *end, const char *key)
{
	static struct strbuf needle = STRBUF_INIT;
	char *bod, *eod, *eol, *cursor;
	 ...
	 * by prefixing continuation lines with a space.  So within
	 * the commit object, it looks like
	 *
			  ...
			  struct string_list *paths_of_changed_objects)
{
	...
	if ((signature = find_commit_signature(committer_end, message, "gpgsig")))
		signature_alg = "sha1";
	else if ((signature = find_commit_signature(committer_end, message, "gpgsig-sha256")))
